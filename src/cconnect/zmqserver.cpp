...
namespace cconnect2 {
  namespace cconnect {
    ...
    ZmqServer::ZmqServer(const std::string ipc) : ipc(ipc) {
    ...
    string ZmqServer::generatePubIPCString(const std::string &name,
                                           const int &pid) const {
      return "ipc://cconnect-" + name + "-" + to_string(pid) + ".ipc";
    }

    string ZmqServer::connectHandle(const Connect &connect,
                                    const string &name,
                                    const int &pid) {
      shared_ptr<zmq::socket_t> sock =
          make_shared<zmq::socket_t>(zmqContext, zmq::socket_type::pub);
